tinymce.PluginManager.add('i_video', function(editor, url) {
    ...
    function selectLocalImages() {
        ...
        if(!editor.settings.video_upload_url){
        ...
        var loading = '<div id="'+ id +'"><span style="display:inline-block;width:10rem;">'+
            ...
            '</span></div>';
        var form = $('<form id="file_upload" enctype="multipart/form-data" action="'+editor.settings.video_upload_url+'" type="post"><input type="hidden" name="uuid" value="'+id+'" /><input accept="video/*" multiple="multiple" type="file" name="file" /></form>');
        form.fileupload({
            dataType:'json',
            ...
            send: function(e,data){
                ...
                if(file.server == ''){
                    $body.find('#' + id).replaceWith($('<div><video uuid="'+id+'" controls controlsList="nodownload" width="100%">' +
                        '  <source src="/' + 'video/index/' + id +'" type="video/mp4">' +
                        'Your browser does not support the video element.' +
                        '</video></div>'));
                }else{
                    $body.find('#' + id).replaceWith($('<div><video uuid="'+id+'" controls controlsList="nodownload" width="100%">' +
                        '  <source src="https://' + file.server + '/' + 'video/index/' + id +'" type="video/mp4">' +
                        'Your browser does not support the video element.' +
                        '</video></div>'));
                }

                //压缩
                // $.postJSON(editor.settings.video_compress_ur, {uuid: id}, function (result) {
                //
                //     $('#' + id).replaceWith($('<p><video controls controlsList="nodownload" controls>' +
                //         '  <source src="https://' + file.server + '/' + result.compress_264 +'" type="video/mp4">' +
                //         'Your browser does not support the video element.' +
                //         '</video></p><br />'));
                // });
