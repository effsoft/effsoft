tinymce.PluginManager.add('i_audio', function(editor, url) {
    ...
    function selectLocalImages() {
        ...
        var loading = '<span id="'+ id +'" style="display:inline-block;">'+
            ...
            '</span>';
        var form = $('<form id="file_upload" enctype="multipart/form-data" action="'+editor.settings.audio_upload_url+'" type="post"><input accept="audio/*" multiple="multiple" type="file" name="file" /></form>');
        form.fileupload({
            dataType:'json',
            ...
            send: function(e,data){
                ...
                $('#' + id).replaceWith($('<p><audio controls controlsList="nodownload" controls>' +
                    '  <source src="'+compress_48+'" type="audio/mp3">' +
